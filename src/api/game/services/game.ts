...
async function getGameInfo(slug) {
  try {
    ...
    const short_description = dom.window.document.querySelector('.description').textContent.slice(0, 160)
    const rating = dom.window.document.querySelector('.age-restrictions__icon use')

    return { 
      description, 
      short_description, 
      rating: rating ? rating.getAttribute('xlink:href').replace(/_/g, '').replace('#', '') : 'BR0'  
    } 
  } catch (error) {
...
export default factories.createCoreService('api::game.game', () => ({
  async populate(params) {
    try {
      const gogApi = `https://catalog.gog.com/v1/catalog?${qs.stringify(
        params
      )}`
